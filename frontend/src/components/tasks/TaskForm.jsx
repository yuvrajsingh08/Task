          ...
          <form onSubmit={saveTask}>
            <div className="mb-4 flex items-start justify-between gap-4">
              <div className="min-w-0">
                ...
                <Dialog.Description className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                  Add the task details, priority, category, and due date.
                </Dialog.Description>
              </div>
              <Dialog.Close className="shrink-0 rounded-full border border-slate-200 bg-white p-2 text-slate-500 transition hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700">
                <X size={18} />
              </Dialog.Close>
            </div>
            ...
            <div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
              {editingId && (
                <button
                  type="button"
                  className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700"
                  onClick={resetForm}>
                  <X size={16} />
                  Clear edit
                </button>
              ...
              <button
                type="submit"
                className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-950 px-5 py-2.5 font-semibold text-white transition hover:bg-slate-800 dark:bg-orange-200 dark:text-slate-950 dark:hover:bg-orange-100">
                <Plus size={18} />
