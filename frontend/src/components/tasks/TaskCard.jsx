...
function TaskCard({ onOpen, task }) {
  ...
  return (
    <article
      className={`group relative flex overflow-hidden rounded-xl border shadow-sm transition-all duration-200 hover:shadow-md ${
        isCompleted
    ...
    >
      <span
        className={`w-1 shrink-0 ${
          isCompleted ? "bg-emerald-400" : priorityStyle.bar
        }`}
        aria-hidden="true"
      />
      ...
      <div className="min-w-0 flex-1 px-2.5 py-2 sm:px-3 sm:py-2.5">
        ...
        <div className="flex min-w-0 items-start gap-2">
          <button
            ...
            className={`mt-0.5 shrink-0 transition ${statusStyle.text} hover:opacity-70`}
          ...
          <div className="min-w-0 flex-1">
            ...
            {task.description ? (
              <p className="mt-1 line-clamp-1 break-words text-[11px] leading-4 text-slate-500 dark:text-slate-400">
                {task.description}
              </p>
            ) : null}
          </div>
        ...
        <div className="mt-1.5 flex flex-col gap-1.5 pl-[25px] sm:flex-row sm:items-center sm:justify-between sm:gap-3">
          ...
          <div className="flex min-w-0 flex-wrap items-center gap-1.5 text-[10px] text-slate-500 dark:text-slate-400">
            <span className="inline-flex min-w-0 max-w-[140px] items-center gap-1 rounded-full bg-[#fff4ed] px-1.5 py-0.5 dark:bg-slate-800">
            ...
            {reminderLabel && (
              <span className="inline-flex items-center gap-1 rounded-full bg-orange-100 px-1.5 py-0.5 text-orange-700 dark:bg-orange-500/15 dark:text-orange-300">
