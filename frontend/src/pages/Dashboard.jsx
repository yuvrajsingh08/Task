...
const getDashboardData = (tasks) => {
  ...
  const stale = incomplete
    .filter((task) => {
      const updatedAt = getTaskDate(task, "updatedAt") || getTaskDate(task, "createdAt");
      return updatedAt && todayStart - updatedAt >= STALE_DAYS * DAY_MS;
    })
    .sort((a, b) => {
      const aDate = getTaskDate(a, "updatedAt") || getTaskDate(a, "createdAt");
      const bDate = getTaskDate(b, "updatedAt") || getTaskDate(b, "createdAt");
      return aDate - bDate;
    });
...
function StatTile({ icon, label, value, surface, iconTone }) {
  return (
    <div
      className={`rounded-lg border border-white/70 p-3 shadow-sm dark:border-slate-800 dark:bg-slate-900 ${
        surface || "bg-orange-50"
      }`}
    >
      <div className="mb-2 flex items-center gap-2 text-xs font-semibold text-slate-700 dark:text-slate-300">
        <span className={iconTone || "text-orange-600 dark:text-orange-300"}>
          {icon}
        </span>
        {label}
      </div>
...
function TaskLine({ task, onOpen }) {
  return (
    <button
      type="button"
      onClick={() => onOpen(task)}
      className="flex w-full items-center justify-between gap-3 rounded-lg border border-orange-100 bg-white px-3 py-2 text-left transition hover:border-orange-200 hover:bg-orange-50 dark:border-slate-800 dark:bg-slate-900 dark:hover:bg-slate-800"
    >
      <span className="min-w-0">
        ...
        <span className="mt-0.5 block text-xs text-slate-500 dark:text-slate-400">
          {task.priority} priority - {formatDate(task.dueDate)}
        </span>
      </span>
...
function Dashboard() {
  ...
  const { aiSummary, loading, pageSize, setPage, setPageSize, tasks } = useTasks();
  ...
  const dashboard = useMemo(() => getDashboardData(tasks), [tasks]);
  const nextTask = useMemo(() => getNextTask(tasks), [tasks]);
  const maxTrend = Math.max(1, ...dashboard.weekTrend.map((item) => item.value));
    ...
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain pb-4">
        <div className="grid gap-3 lg:grid-cols-[1.1fr_0.9fr]">
          <section className="rounded-xl border border-orange-100 bg-white p-4 shadow-sm shadow-orange-100/50 dark:border-slate-800 dark:bg-slate-900 dark:shadow-none">
          ...
          <section className="rounded-xl border border-orange-100 bg-white p-4 shadow-sm shadow-orange-100/50 dark:border-slate-800 dark:bg-slate-900 dark:shadow-none">
            ...
            <div className="grid gap-2">
              {attentionItems.map((item) => (
                <div
                  key={item.label}
                  className="flex items-center justify-between gap-3 rounded-lg border border-orange-100 bg-orange-50 px-3 py-2.5 dark:border-slate-800 dark:bg-slate-800"
                >
                  <div className="flex min-w-0 items-center gap-2">
                    ...
                    >
                      {item.task ? item.icon : <CheckCircle2 size={16} />}
                    </span>
                    <span className="min-w-0">
                      ...
                      <span className="block truncate text-xs text-slate-500 dark:text-slate-400">
                        {item.task ? item.helper : item.emptyHelper}
                      </span>
                    </span>
                  </div>
                  {item.task ? (
        ...
        <div className="mt-3 grid gap-3 xl:grid-cols-[1fr_0.8fr]">
          <section className="rounded-xl border border-orange-100 bg-white p-4 shadow-sm shadow-orange-100/50 dark:border-slate-800 dark:bg-slate-900 dark:shadow-none">
            ...
            <div className="mt-4 rounded-lg border border-orange-100 p-3 dark:border-slate-800">
              ...
              <div className="flex h-32 items-end gap-2">
                {dashboard.weekTrend.map((item, index) => (
                  <div key={`${item.label}-${index}`} className="flex min-w-0 flex-1 flex-col items-center gap-1">
                    <div className="flex h-20 w-full items-end rounded-md bg-[#fff4ed] dark:bg-slate-800">
                      <div
                        ...
                        style={{
                          height:
                            item.value > 0
                              ? `${Math.max(18, (item.value / maxTrend) * 100)}%`
                              : "0%",
                        }}
          ...
          <section className="rounded-xl border border-orange-100 bg-white p-4 shadow-sm shadow-orange-100/50 dark:border-slate-800 dark:bg-slate-900 dark:shadow-none">
            <div className="mb-4 flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  Incomplete tasks untouched for {STALE_DAYS}+ days.
                </p>
              </div>
            </div>
            ...
            {dashboard.stale.length > 0 ? (
              <div className="grid gap-2">
                {dashboard.stale.slice(0, 4).map((task) => (
                  <TaskLine key={task._id} task={task} onOpen={setSelectedTask} />
                ))}
              </div>
            ) : (
              <p className="rounded-lg bg-orange-50 px-3 py-4 text-sm text-slate-600 dark:bg-slate-800 dark:text-slate-300">
                No stale incomplete tasks right now.
              </p>
            )}
          </section>
        </div>
